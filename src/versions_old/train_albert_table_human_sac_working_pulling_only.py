...
class AlbertTableImpedanceSim:
    ...
    def human_follower_impedance_controller(self, F_robot):
        """ Simple 'human follower' impedance controller. """
        if self.human_goal_link_idx is None:
            return

        human_alpha = 0.6
        force_ema_beta = 0.25
        human_F_max = np.array([120.0, 120.0, 120.0])
        yaw_damping_gain = 3.0

        if not hasattr(self, "_human_force_ema"):
            self._human_force_ema = np.zeros(3)

        self._human_force_ema = (
            (1 - force_ema_beta) * self._human_force_ema + force_ema_beta * F_robot
        )

        F_human = human_alpha * self._human_force_ema
        F_human = np.clip(F_human, -human_F_max, human_F_max)

        handle_state_H = p.getLinkState(self.table_id,
                                        self.human_goal_link_idx,
                                        computeLinkVelocity=0)
        handle_pos_H = np.array(handle_state_H[0])

        # p.applyExternalForce(
        #     self.table_id, self.human_goal_link_idx,
        #     F_human.tolist(), handle_pos_H.tolist(),
        #     flags=p.WORLD_FRAME,
        # )

        _, _, _, wz = self.get_table_state_world()
        # p.applyExternalTorque(
        #     self.table_id, -1,
        #     [0, 0, -yaw_damping_gain * wz],
        #     flags=p.WORLD_FRAME
        # )
    ...
    def impedance_step(self):
        ...
        self.human_follower_impedance_controller(F)
...
class AlbertTableEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        # Robot → Table geometry (critical!)
        dx_rg, dy_rg = goal_xy - robot_pos
    ...
    def compute_reward(self, dist, prev_dist, heading_error_bi, heading_diff):
        ...
        # basicly times 
        kpr = 18.0 # used to be twelf
        progress_reward = kpr * (prev_dist - dist) / 0.01 

        # added stagnation penalty so robot does gets stuck in local optimum near goal
...
if __name__ == "__main__":
    ...
    model_name = "28_okt_test_fix"
    base_log_dir = "runs_28_okt_test_fix"
    render = False
