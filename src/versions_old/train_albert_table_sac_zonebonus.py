...
class AlbertTableEnv(gym.Env):
    ...
    def __init__(self, render=False, max_steps=1000, goals=None):
        ...
        # bonus flags
        self.bonus_given_far = False
        self.bonus_given_mid = False
    ...
    def compute_reward(self, dist, prev_dist, heading_error_bi, heading_diff):
        ...
        progress_reward = (prev_dist - dist) * 150

        # Penalty for being far from goal
        distance_penalty = -dist 

        # Penalize misalignment toward goal
        heading_penalty = -1.0 * heading_error_bi

        # NEW: Penalize large yaw difference between robot and table
        alignment_heading_penalty = -0.5 * abs(heading_diff)

        # NEW: Small bonus for perfect alignment (both facing same direction)
        # alignment_bonus = 0.2 * np.exp(-5.0 * abs(heading_diff))
        alignment_bonus = 0
        

        total_reward = (progress_reward + distance_penalty +
                        heading_penalty + alignment_heading_penalty + alignment_bonus)
        
        #print("heading_diff", heading_diff)
        #print(f"progress_reward: {progress_reward}, distance_penalty: {distance_penalty}, heading_penalty: {heading_penalty}, alignment_heading_penalty: {alignment_heading_penalty}, alignment_bonus: {alignment_bonus}")
        return total_reward, progress_reward, distance_penalty, heading_penalty, alignment_heading_penalty, alignment_bonus
    ...
    def step(self, action):
        ...
        # --- Compute reward
        reward, progress_reward, distance_penalty, heading_penalty, alignment_heading_penalty, alignment_bonus = \
            self.compute_reward(dist, self.prev_dist, heading_error_bi, heading_diff)
        self.prev_dist = dist

        # 1. Far zone: first time entering < 1.5 m
        if not self.bonus_given_far and dist < 1.5:
            print("far close bonus is given")
            reward += 5.0
            self.bonus_given_far = True

        # 2. Mid zone: first time entering < 0.75 m
        if not self.bonus_given_mid and dist < 0.75:
            print("mid close bonus is given")
            reward += 15.0
            self.bonus_given_mid = True
