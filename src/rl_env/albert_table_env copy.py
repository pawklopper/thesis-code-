...
class AlbertTableEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        # Note: Position is still read raw, but yaw is corrected
        ...
        # ---------------------------------------
        # Compare World-Frame Table Yaw to World-Frame Robot Yaw
    ...
    #     # Corrected for WF: use standard [cos, sin]
    #     robot_yaw = self.get_robot_yaw_wf()
    #     heading_vec = np.array([np.cos(robot_yaw), np.sin(robot_yaw)])
    ...
    def apply_progressive_leash(self, action):
        ...
        # 1. Unit Heading Vector (0 deg = X_world)
        # Corrected yaw uses standard trig [cos, sin]
        ...
        # Tangential velocity = [-y, x] * w relative to arm
        # Note: r_arm is already in world frame coordinates relative to robot center
