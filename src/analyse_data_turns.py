...
CONFIG = {
    ...
    "wz_eps": 0.05,                             # deadband for wz to suppress noisy sign flips (rad/s)
...
def main():
    ...
    if not summary_df.empty:
        fig, (ax1, ax3, ax5, ax7) = plt.subplots(4, 1, figsize=(14, 22))
        ...
        ymax = max(1.0, float(np.max(n_turns)) * 1.1)
        ax1_count.set_ylim(0, ymax)
        ...
        # --- SUBPLOT 4: AVG FORCE MAGNITUDES ---
        ax7.plot(x, summary_df["avg_fh_force"].to_numpy(), color='#1f77b4', marker='o', linestyle='-', label="Avg |Fh|")
        ax7.plot(x, summary_df["avg_fr_force"].to_numpy(), color='#ff7f0e', marker='x', linestyle='-', label="Avg |Fr|")
        ax7.set_ylabel("Avg Force (N)", fontsize=11)
        ax7.set_xticks(x); ax7.set_xticklabels(episode_labels)
        ax7.set_xlabel("Episode", fontsize=12)
        ax7.legend(loc="upper left", bbox_to_anchor=(1.08, 1))
